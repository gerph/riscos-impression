...
def test_paragraph_css_properties_positive_right_indent_needs_a_max_width_to_apply_at_all():
    # Regression test: a POSITIVE right_indent_raw (Style.
    # right_indent_is_delta True; DDL kind 2, INDENTOFFSET in the
    # conversion source) is an offset from the frame's own LEFT edge,
    # not an inset from its right -- confirmed against a real document
    # (PCI_Spec from the local examples/ corpus) and a real DDF export
    # the user supplied, generated by Impression itself. Resolving it
    # to a CSS margin-right (an inset from the box's own right edge)
    # needs the box's own real width; scrolling HTML deliberately never
    # tracks one (it drops absolute geometry -- see html_scrolling.py's
    # own module docstring), so it must never apply there at all.
...
def test_paragraph_css_properties_positive_right_indent_applies_within_a_known_width():
    # right_indent_raw=5000 (5pt) is an offset from the frame's own
    # left edge; within a 100pt-wide frame, that resolves to a
    # margin-right of 100 - 5 = 95pt (the column runs from X=0 to
    # X=5pt), not 5pt itself.
    style = _style(1, is_body_text=True, right_indent_raw=5000)
    props = paragraph_css_properties(style, max_width_pt=100.0)
    assert props["margin-right"] == "95.00pt"


def test_paragraph_css_properties_negative_right_indent_applies_directly_as_a_margin():
    # A NEGATIVE right_indent_raw (right_indent_is_delta False; DDL
    # kind 0) is a genuine inset from the frame's own right edge --
    # maps directly to CSS margin-right, and (unlike the positive case)
    # needs no frame width to resolve, so it applies even in scrolling
    # HTML (max_width_pt=None).
    style = _style(1, is_body_text=True, right_indent_raw=-5000)
    assert paragraph_css_properties(style)["margin-right"] == "5.00pt"
    assert paragraph_css_properties(style, max_width_pt=100.0)["margin-right"] == "5.00pt"


def test_paragraph_css_properties_positive_right_indent_smaller_than_left_indent_is_dropped():
    # A positive right_indent (an offset from the frame's own left
    # edge) smaller than the frame's own width but still yielding a
    # negative/zero margin-right (i.e. genuinely narrower than the
    # left_indent already claims) is dropped rather than emitting a
    # nonsensical negative CSS margin.
    style = _style(1, is_body_text=True, left_indent=20000, right_indent_raw=5000)
    props = paragraph_css_properties(style, max_width_pt=3.0)
