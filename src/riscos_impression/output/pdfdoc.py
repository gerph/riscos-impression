...
def _line_height_pt(style: Style) -> float:
    ...
    if style.line_spacing_raw is not None:
        if style.line_spacing_is_fixed:
            ...
            # DDF export the user supplied, generated by Impression
            # itself): its "Main Heading" style (28pt) carries a fixed
            # leading of 19.66pt
...
class PDFConverter(Converter):
    ...
    def _flow_paragraphs_into_containers(
        ...
        for paragraph in paragraphs:
            ...
            # Style.right_indent_is_delta and docs/impression-
            # documents.xml's "Paragraph ruler fields"): a POSITIVE raw
            # value (right_indent_is_delta True; DDL kind 2,
            # INDENTOFFSET in the conversion source) is an offset from
            # the frame's own LEFT edge, not an inset from its right --
            # confirmed against a real document (PCI_Spec from the
            # local examples/ corpus) and a real DDF export the user
            # supplied, generated by Impression itself: its base "TextStyle"
            # declares leftmargin 19.8pt, rightmargin 510.2pt on a
            # frame that's itself 510.2pt wide -- read as an offset
            # from the left, rightmargin lands almost exactly at the
            # frame's own right edge (a normal, narrow margin); read as
            # an inset from the right (this project's own prior,
            # incorrect assumption), it left less than a point of usable
            # width on every line. A non-positive raw value (kind 0) is
            # the inset-from-the-right-edge case this project always
            # assumed applied universally. Resolved to an absolute X
            # (see right_edge() below) once cx0/cx1 for the current
            # container are known, since a paragraph can span more than
            # one container and each has its own box.
            right_indent_pt = (para_style.right_indent or 0) / UNIT
            right_indent_is_delta = para_style.right_indent_is_delta

            def resolve_right_edge(cx0: float, cx1: float) -> float:
                return (cx0 + right_indent_pt) if right_indent_is_delta else (cx1 - right_indent_pt)
            ...
            while tokens or not placed_any_line:
                ...
                if tokens and tokens[0].kind == "embed":
                    ...
                    while True:
                        ...
                        avail_x0 = cx0 + indent
                        avail_x1 = resolve_right_edge(cx0, cx1)
                ...
                right_edge = resolve_right_edge(cx0, cx1)
                if right_edge - line_start < _MIN_USABLE_WIDTH:
                    # A genuinely degenerate style (e.g. a left_indent
                    # so large, or a kind-0 right_indent so large, that
                    # no usable width is left at all -- see
                    # resolve_right_edge above for the two very
                    # different things right_indent can mean) leaves no
                    # room for this paragraph in this container at all.
                    # Falling back to the container's own full width
                    # (dropping only the right margin, keeping
                    # line_start as computed, so a real hanging
                    # first-line indent survives) is safer than the
                    # alternative: with tokens never consumed, the
                    # "obstacle leaves no room" skip below would burn
                    # through this container -- and every later chain
                    # member -- without ever placing this paragraph,
                    # silently dropping it and every paragraph after it
                    # in the whole story, not just producing one
                    # badly-indented line.
